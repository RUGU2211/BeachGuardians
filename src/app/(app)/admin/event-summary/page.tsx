...
import { useCleanupSummarizer } from '@/hooks/use-ai-api';
...
export default function EventSummaryPage() {
  ...
  const { loading: isLoading, data: summaryData, callAI: generateSummary } = useCleanupSummarizer();
  ...
  const onSubmit = async (data: EventSummaryFormValues) => {
    const result = await generateSummary('summarize-cleanup-event', data);
    if (result) {
      toast({
        title: 'Summary Generated!',
        description: 'Event summary has been generated successfully.',
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      {summaryData && (
        <Card>
          ...
          <CardContent>
            <Textarea value={summaryData.summary} readOnly rows={8} className="bg-muted/50 whitespace-pre-wrap" />
            <div className="mt-2 text-xs text-muted-foreground">
              Generated by {summaryData.generatedBy} on {new Date(summaryData.generatedAt).toLocaleString()}
            </div>
          </CardContent>
        </Card>
      )}
