...
import { getEventById } from '@/lib/mockData'; // Assuming this function exists
...
export default function AiEventSummaryWithIdPage() {
  ...
  const form = useForm<EventSummaryFormValues>({
    ...
    defaultValues: {
      ...
      notableObservations: 'Volunteers were very enthusiastic. Found a significant amount of fishing debris.',
    },
  });
    ...
    if (eventId) {
      ...
      if (event) {
        form.reset({
          ...
          totalVolunteers: event.volunteers.length,
          totalWasteCollectedKg: event.wasteCollectedKg || 0,
          typesOfWasteCollected: 'Please specify types of waste', // Placeholder
          notableObservations: `Summary for ${event.name}: `, // Placeholder
        });
      } else {
         toast({ title: 'Error', description: `Event with ID ${eventId} not found.`, variant: 'destructive'});
      }
    }
  }, [eventId, form, toast]);
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {eventId && ` Pre-filled for event: ${form.getValues('eventName') || eventId}`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
